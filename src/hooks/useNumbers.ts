...
/**
  * @author #Dev.Youssef
  * @description This hook used to create ( Random Numbers, get reuslt of array, .....)
  * @github https://github.com/dsc-youssef
  * @facebook https://www.facebook.com/YoussefBibawy1/
*/
const useNumbers = () => {
  /**
    * @desc This function used to create an array of random numbers.
    * @param { number } min
    * @param { number } max
    * @param { boolean } floating
    * @return number[]
  */
  const randNumbers = (min: number, max: number, floating = true, length:number = 1): number[] => Array.from({ length }, () => (floating ? Math.random() * (max - min) + min : Math.floor(Math.random() * (max - min + 1) + min)));
 
  /**
   * @desc This function used to calculate array numbers and return result.
  ...
  */
  ...
  /** 
   * @desc This function used to change number format.
  ...
  */
  ...
  /**
   * @desc This function used to get the result percent of total number.
